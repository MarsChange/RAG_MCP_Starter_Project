...
3. **Agent 设置**：初始化 MCP 客户端（fetch、filesystem）
4. **任务执行**：LLM 使用工具处理任务
5. **工具调用**：Agent 根据需要自动调用工具
...
### 异步上下文管理器错误
确保您使用的是 Python 3.12+ 版本，并且所有 MCP 客户端都在异步上下文管理器中正确管理。
...
修改 `utils/embedding.py` 以使用不同的嵌入模型或提供商。
